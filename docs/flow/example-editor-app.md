...
A tela é uma `Column`: `EditorTopBar` (reset, menu de origem da timeline e resolução 720p/1080p), `PreviewArea` (`Texture` + pan/crop por arrasto + ghost de texto arrastável), `EditorToolbar` (play circular + tempo, dividir/velocidade/proporção/texto/excluir, undo/redo, slider de zoom e export com percentual — rolável horizontalmente em telas estreitas), `TimelineSection` (coluna fixa de lane headers + régua, faixa de clipes, faixa de textos, faixa de áudio e playhead em scroll horizontal) e uma status bar condicional no fim.
...
12. **Trim** — `example/lib/editor/widgets/clip_trim_handles.dart`
 ...
 13. **Toolbar** — `example/lib/editor/widgets/editor_toolbar.dart`
      Dividir → `controller.split(state)`; Velocidade → `showSpeedSheet` (slider 0.5–2.0 + chips, commit em `onChangeEnd`); Proporção → `showAspectRatioSheet`; Texto → `controller.addTextOverlay()` (overlay default "Texto" na posição atual de playback, janela de 3 s) + `showTextEditSheet`; Excluir → `removeSelected` (só com mais de um clipe).
...
18. **Encerramento** — `EditorScreen.dispose` → `EditorController.dispose`
    Cancela o timer de seek, a assinatura de estado e dispõe o `NativeTimelinePlayer`.
...
| Estado | `example/lib/editor/editor_controller.dart` | Toda a orquestração do plugin e o espelho local de clipes, áudio e textos |
...
| Widget | `example/lib/editor/widgets/editor_toolbar.dart` | Play circular + tempo, dividir/velocidade/proporção/texto/excluir, undo/redo, slider de zoom e export com progresso (linha única rolável) |
...
- **Edição commita no release** — trim, velocidade, volume de áudio e textos só chamam o nativo no fim do gesto, porque cada comando reconstrói a composição (Android) ou re-renderiza a videoComposition (iOS). O ghost do preview atualiza só a cópia local durante o arrasto.
