...
> **Resumo:** O app em `example/` é um editor de vídeo no estilo CapCut que consome o plugin: preview em `Texture`, timeline com régua/playhead/faixa de clipes, edição por gesto e export para a galeria.

## Visão Geral

`main.dart` monta `TimelineEditorApp` (um `MaterialApp` com `editorTheme` escuro) e abre `EditorScreen(autoLoad: true)`. A tela cria um `EditorController` no `initState` e, no primeiro frame, chama `loadSample()`.
...
A tela é uma `Column`: `EditorTopBar` (reset, menu de origem da timeline, resolução 720p/1080p e botão de export com percentual), `PreviewArea` (`Texture` + pan/crop por arrasto), `PlaybackBar` (play/pause, tempo, undo/redo), `TimelineSection` (régua, faixa de clipes, faixa de áudio, playhead, zoom) e `BottomToolbar` (dividir, velocidade, proporção, excluir), com uma status bar condicional no fim.
...
8. **Playback** — `example/lib/editor/widgets/playback_bar.dart` → `controller.playOrPause(state)`
...
9. **Timeline** — `example/lib/editor/widgets/timeline_section.dart`
   `contentWidth` derivado de `pixelsPerSecond`; empilha `TimelineRuler`, `ClipStrip`, `AudioTrackRow` e `TimelinePlayhead` num `Stack` dentro de scroll horizontal; header com tempo e slider de zoom.
...
13. **Toolbar** — `example/lib/editor/widgets/bottom_toolbar.dart`
...
16. **Export** — `EditorTopBar._ExportButton` → `controller.export`
...
17. **Undo/redo** — `PlaybackBar` → `controller.undo()` / `redo()`
...
| Tema | `example/lib/editor/theme/editor_theme.dart` | Paleta (`editorAccent`, `editorBackground`, `editorSurface`, `editorSurfaceHigh`, `editorLine`, `editorTextMuted`) e `editorTheme` |
| Widget | `example/lib/editor/widgets/editor_top_bar.dart` | Reset, origem da timeline, resolução, botão de export com progresso |
| Widget | `example/lib/editor/widgets/preview_area.dart` | `Texture`, `AspectRatio`, pan/crop por gesto, placeholder |
| Widget | `example/lib/editor/widgets/playback_bar.dart` | Play/pause, tempo formatado, undo/redo |
| Widget | `example/lib/editor/widgets/timeline_section.dart` | Layout da timeline, scroll, zoom, scrub do playhead |
| Widget | `example/lib/editor/widgets/timeline_ruler.dart`, `timeline_ruler_painter.dart` | Régua de segundos e seu `CustomPainter` |
| Widget | `example/lib/editor/widgets/timeline_playhead.dart` | Agulha arrastável |
...
| Widget | `example/lib/editor/widgets/bottom_toolbar.dart` | Dividir, Velocidade, Proporção, Excluir |
...
- **Undo exige acordo entre nativo e local** — a `PlaybackBar` usa `state.canUndo && controller.canUndo`; o controller mantém `_undoSnapshots`/`_redoSnapshots` com limite de 50.
