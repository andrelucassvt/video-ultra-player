...
- [x] Em `makeEditSnapshot()`/`restoreEditSnapshot(_:)`, incluir `textOverlays`
- [x] Em `ios/Classes/TimelineEditModel.swift`, adicionar `let textOverlays: [TextOverlayDescriptor]` a `TimelineEditSnapshot` e atualizar o init memberwise
- [x] Verificação: `cd example && flutter build ios --no-codesign` compila (build apenas — não executar o app). Se o ambiente não tiver CocoaPods configurado, registrar o bloqueio e seguir — a verificação mínima é ausência de erros de sintaxe nos arquivos alterados

### Fase 2 — Renderização (CALayer/CATextLayer)

- [x] Criar `ios/Classes/TextOverlayLayers.swift` com:
...
- [x] Em `TimelineComposition.makeVideoComposition()`, ao final: se `!textOverlays.isEmpty`, montar `videoLayer` (frame = renderSize) + `outputLayer` (frame = renderSize, contém `videoLayer` e a parent layer de texto) e atribuir `videoComposition.animationTool = AVVideoCompositionCoreAnimationTool(postProcessingAsVideoLayer: videoLayer, in: outputLayer)`
- [x] Verificação: `flutter build ios --no-codesign` compila

### Fase 3 — Mutações no controller + routing

- [x] Em `ios/Classes/VideoUltraPlayerPlugin.swift`, adicionar ao `TimelinePlayerController` (após `removeAudioTrack`, linha ~690), seguindo o padrão de `setClipAlignment` (snapshot → mutate → re-gerar videoComposition → reatribuir):
  ```swift
...
- [x] Adicionar `private func applyUpdatedVideoComposition()`: re-gerar via um novo método internal `composition.updatedVideoComposition()` (wrapper de `makeVideoComposition()`), atribuir a `player.currentItem?.videoComposition`, e — se `player.rate == 0` — fazer seek de tolerância zero na posição atual + `texture.requestFrame()` para o frame pausado refletir o texto imediatamente; `emitState()` ao final
- [x] Adicionar os 3 cases no `handle(_:result:)` (após `removeAudioTrack`, linha ~214):
...
- [x] Verificação: `flutter build ios --no-codesign` compila; revisar que `exportCurrentTimeline` não precisa de alteração (o `animationTool` já vai na `videoComposition` exportada — confirmar lendo `runExportSession`, linha ~364)
- [x] Checkpoint: commit das mudanças da parte + informar o usuário que a parte 2 está concluída e a parte 3 está pronta para execução

## Critérios de Sucesso

- [x] Plugin iOS compila sem erros (`flutter build ios --no-codesign`)
- [x] `makeVideoComposition()` é o único ponto de criação de videoComposition e inclui o `animationTool` quando há textos
- [x] Undo/redo restauram textos (snapshot inclui `textOverlays` e o rebuild passa pelo mesmo `makeVideoComposition()`)
- [x] `exportTimeline(clips)` (sem player) segue exportando sem textos — paridade com a trilha de áudio
