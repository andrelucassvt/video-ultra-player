...
class EditorController extends ChangeNotifier {
  EditorController({NativeTimelinePlayer? player})
  ...
  /// Loads the clips chosen on the media-picker screen into the timeline.
  ///
  /// Turning the user's selected files into [TimelineClip]s is the picker's
  /// job (see `media/media_clip.dart`); from here the controller owns the
  /// editing session — split, trim, reorder, speed, audio, export, undo/redo.
  Future<void> loadClips(List<TimelineClip> clips) async {
    if (_loading || clips.isEmpty) return;
    _setBusy(loading: true);

    try {
      await replaceTimeline(clips, source: 'Seleção');
    } catch (error) {
      _setError(error);
